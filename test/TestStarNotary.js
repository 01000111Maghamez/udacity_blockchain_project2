...
it('lets user2 buy a star and decreases its balance in ether', async() => {
    ...
    assert.equal(Number(balanceOfUser2BeforeTransaction) - starPrice - tx.receipt.cumulativeGasUsed * tx.receipt.effectiveGasPrice, Number(balanceAfterUser2BuysStar));
});
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('Awesome Star!', starId2, {from: accounts[1]})
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let starId = 10;
    let instance = await StarNotary.deployed();
    await instance.createStar('A New Star!', starId, {from: accounts[0]})
    assert.equal(await instance.lookUptokenIdToStarInfo.call(starId), 'A New Star!')
});